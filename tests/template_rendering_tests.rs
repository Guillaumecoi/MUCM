...
fn create_full_test_data() -> HashMap<String, Value> {
    ...
    data.insert("description".to_string(), json!("A comprehensive test use case"));
    data.insert("summary".to_string(), json!("Brief summary of the test use case"));
    ...
    data.insert("preconditions".to_string(), json!([
        "User is authenticated",
        "System is online"
    ]));
    
    data.insert("postconditions".to_string(), json!([
        "Data is saved",
        "User receives confirmation"
    ]));
    
    data.insert("use_case_references".to_string(), json!([
        {
    ...
    data.insert("business_value".to_string(), json!("Increases user engagement by 25%"));
    data.insert("stakeholders".to_string(), json!(["Product Manager", "CTO"]));
    data.insert("success_criteria".to_string(), json!(["95% uptime", "< 2s response time"]));
    
    // Developer methodology fields
    data.insert("technical_requirements".to_string(), json!("RESTful API with OAuth2"));
    data.insert("api_endpoints".to_string(), json!(["/auth/login", "/auth/validate"]));
    data.insert("dependencies".to_string(), json!(["auth-service", "database"]));
    data.insert("error_handling".to_string(), json!("Return 401 for invalid credentials"));
    
    // Feature methodology fields
    data.insert("user_segment".to_string(), json!("Enterprise users"));
    data.insert("success_metrics".to_string(), json!(["NPS > 8", "Engagement +30%"]));
    data.insert("hypothesis".to_string(), json!("Users need faster authentication"));
    data.insert("feature_dependencies".to_string(), json!(["SSO integration"]));
    data.insert("design_assets".to_string(), json!(["https://figma.com/design/123"]));
    ...
    data.insert("regression_suite".to_string(), json!("Auth Regression Suite"));
    data.insert("performance_baseline".to_string(), json!("< 500ms response time"));
    data.insert("coverage_areas".to_string(), json!(["Authentication", "Authorization"]));
    data.insert("test_data_requirements".to_string(), json!("Test user accounts"));
...
fn test_business_normal_template() {
    ...
    assert!(result.is_ok(), "Business normal template should render: {:?}", result.err());
...
fn test_business_advanced_template() {
    ...
    assert!(result.is_ok(), "Business advanced template should render: {:?}", result.err());
    let rendered = result.unwrap();
    assert!(rendered.contains("UC-TST-001"));
    assert!(rendered.contains("Test Use Case"));
}
...
fn test_developer_normal_template() {
    ...
    assert!(result.is_ok(), "Developer normal template should render: {:?}", result.err());
    let rendered = result.unwrap();
    assert!(rendered.contains("UC-TST-001"));
    assert!(rendered.contains("Test Use Case"));
}
...
fn test_developer_advanced_template() {
    ...
    assert!(result.is_ok(), "Developer advanced template should render: {:?}", result.err());
    let rendered = result.unwrap();
    assert!(rendered.contains("UC-TST-001"));
    assert!(rendered.contains("Test Use Case"));
}
...
fn test_feature_normal_template() {
    ...
    assert!(result.is_ok(), "Feature normal template should render: {:?}", result.err());
    let rendered = result.unwrap();
    assert!(rendered.contains("UC-TST-001"));
    assert!(rendered.contains("Test Use Case"));
}
...
fn test_feature_advanced_template() {
    ...
    assert!(result.is_ok(), "Feature advanced template should render: {:?}", result.err());
    let rendered = result.unwrap();
    assert!(rendered.contains("UC-TST-001"));
    assert!(rendered.contains("Test Use Case"));
}
...
fn test_tester_normal_template() {
    ...
    assert!(result.is_ok(), "Tester normal template should render: {:?}", result.err());
    let rendered = result.unwrap();
    assert!(rendered.contains("UC-TST-001"));
    assert!(rendered.contains("Test Use Case"));
}
...
fn test_tester_advanced_template() {
    ...
    assert!(result.is_ok(), "Tester advanced template should render: {:?}", result.err());
    let rendered = result.unwrap();
    assert!(rendered.contains("UC-TST-001"));
    assert!(rendered.contains("Test Use Case"));
}
...
fn test_all_templates_with_minimal_data() {
    ...
    for methodology in methodologies {
        for level in levels.iter() {
            let result = engine.render_use_case_with_methodology_and_level(&data, methodology, level);
            assert!(
                result.is_ok(), 
                "{}-{} template should handle minimal data: {:?}", 
                methodology, level, result.err()
            );
        }
    }
}
...
fn test_rust_test_template() {
    ...
    assert!(result.is_ok(), "Rust test template should render: {:?}", result.err());
    let rendered = result.unwrap();
    assert!(rendered.contains("UC-TST-001"));
...
fn test_python_test_template() {
    ...
    assert!(result.is_ok(), "Python test template should render: {:?}", result.err());
    let rendered = result.unwrap();
    assert!(rendered.contains("UC-TST-001"));
...
fn test_javascript_test_template() {
    ...
    assert!(result.is_ok(), "JavaScript test template should render: {:?}", result.err());
    let rendered = result.unwrap();
    assert!(rendered.contains("UC-TST-001"));
...
fn test_scenario_partial_rendering() {
    ...
    assert!(result.is_ok(), "Template with scenario partial should render");
    let rendered = result.unwrap();
...
fn test_templates_with_empty_scenarios() {
    ...
    for methodology in methodologies {
        let result = engine.render_use_case_with_methodology_and_level(&data, methodology, "normal");
        assert!(
            result.is_ok(),
            "{} template should handle empty scenarios: {:?}",
            methodology, result.err()
        );
    }
}
...
fn test_overview_template() {
    ...
    assert!(result.is_ok(), "Overview template should render: {:?}", result.err());
    let rendered = result.unwrap();
...
fn test_all_templates_comprehensive() {
    ...
    for methodology in &methodologies {
        for level in &levels {
            let result = engine.render_use_case_with_methodology_and_level(&full_data, methodology, level);
            assert!(
                result.is_ok(),
    ...
    for methodology in &methodologies {
        for level in &levels {
            let result = engine.render_use_case_with_methodology_and_level(&minimal_data, methodology, level);
            assert!(
                result.is_ok(),
